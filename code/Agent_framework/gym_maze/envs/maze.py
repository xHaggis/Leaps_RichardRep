...
class MazeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self._goal_test(self.state):  # Goal check
            ...
            reward = 0  # ARMAND : I changed the value to 0, was -0.01 before
            done = 0
        
        # Additional info
        # info = {}
        
        return self._get_obs(), reward, done, self.state  # info

    def look(self, direction):
        current_location = self.state
        look_direction = self._next_state(self.state, direction)

        if self._goal_test(self.state):  # Goal check
            reward = 1
            done = 1
        elif look_direction == current_location:  # Hit wall
            # print('I hit a wall')
            reward = -1
            done = 0
        else:
    ...
    def _next_state(self, state, action):
        ...
        if self.action_type == 'VonNeumann':
            ...
            # ARMAND : Changed the direction, was 0Up, 1Down, 2Left, 3Right
